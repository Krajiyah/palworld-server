...
- ✅ **Cost-Optimized**: EC2 Spot instances (~70% cheaper) with auto-recovery
- ✅ **Zero Data Loss**: Persistent EBS volume survives Spot interruptions
- ✅ **Static IP**: Elastic IP address stays constant across interruptions
- ✅ **Auto-Generated Passwords**: Terraform creates strong random passwords
- ✅ **Secrets Manager Integration**: Passwords stored securely in AWS Secrets Manager
- ✅ **Automatic IP Detection**: SSH auto-restricted to your current IP
- ✅ **Automated Backups**: S3 backups every 6 hours + Spot interruption handling
- ✅ **Monitoring Dashboard**: Real-time CPU, memory, and player count metrics
- ✅ **Cross-Platform**: Supports Steam, Xbox Game Pass PC, Windows Store
- ✅ **Secure**: ED25519 SSH keys, auto-detected IP restrictions
- ✅ **One-Command Deploy**: `terraform apply` to create, `terraform destroy` to remove
- ✅ **Remote State**: Optional S3 backend for state storage
...
|----------|-----------|
| Steam (PC) | ✅ Yes |
| Xbox Game Pass (PC) | ✅ Yes |
| Windows Store (PC) | ✅ Yes |
| PS5 Console | ❌ No (dedicated servers not supported by Palworld) |
| Xbox Console | ❌ No (dedicated servers not supported by Palworld) |
...
**🔐 Passwords**: Auto-generated by Terraform! No manual entry needed.
**🔒 SSH Security**: Set `restrict_ssh_to_current_ip = true` to automatically restrict SSH access to your current public IP.
...
**⚠️ Warning**: All players will need the new password! Secrets Manager is automatically updated.
...
|------|---------|-------------|
| 22 | SSH | ✅ Your IP only |
| 25575 | RCON | ✅ Your IP only |
| 8211 | Game Server | ❌ Open to all (players need this) |
| 27015 | Steam Query | ❌ Open to all |
| 80 | Dashboard | ❌ Open to all |
...
- ✅ **Disaster Recovery**: Survive local machine loss
- ✅ **State Backup**: Automatic backups via S3 versioning
- ✅ **Encryption**: State encrypted at rest
- ✅ **Team Collaboration**: Multiple developers can work together (if needed later)
- ✅ **Cost**: < $0.02/month
...
**⚠️ WARNING**: This deletes your game saves! Manually backup S3 bucket first if needed.
...
```
palworld-server/
├── main.tf                    # Core Terraform config, providers, passwords
├── variables.tf               # Input variables
├── outputs.tf                 # Output values
├── data.tf                    # Data sources (IP detection)
├── network.tf                 # VPC, subnets, security groups
├── compute.tf                 # EC2, ASG, Launch Template
├── storage.tf                 # EBS volume, S3 bucket, Elastic IP
├── secrets.tf                 # AWS Secrets Manager for passwords
├── iam.tf                     # IAM roles and policies
├── lambda_functions.tf        # Lambda resources
├── monitoring.tf              # CloudWatch, EventBridge
├── backend.tf                 # Remote state configuration
├── backend-config.tf          # S3 backend setup (commented)
...
├── lambda/
│   ├── volume_attachment.py   # EBS attachment handler
│   └── spot_interruption.py   # Spot interruption handler
├── dashboard/
│   ├── index.html             # Monitoring dashboard UI
│   └── dashboard.js           # Dashboard JavaScript
└── config/
    └── PalWorldSettings.ini.example  # Server settings reference
```
